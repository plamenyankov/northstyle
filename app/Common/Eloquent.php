...
use \DataObject\NorthstyleDate as NorthstyleDate;
...
class Eloquent extends EloquentModel {
	...
	public function fromDateTime($value)
	{
		...
		{
			$value = NorthstyleDate::createFromTimestamp($value);
		}
		...
		{
			$value = NorthstyleDate::createFromFormat('Y-m-d', $value)->startOfDay();
		}
		...
		{
			$value = NorthstyleDate::createFromFormat($format, $value);
		}
	...
	{		
		...
		if (is_numeric($value))
		{
			return NorthstyleDate::createFromTimestamp($value);
		}
		...
		elseif (preg_match('/^(\d{4})-(\d{2})-(\d{2})$/', $value))
		{
			return NorthstyleDate::createFromFormat('Y-m-d', $value)->startOfDay();
		}
		...
		elseif ( ! $value instanceof DateTime)
		{
			...
			return NorthstyleDate::createFromFormat($format, $value);
		}

		return NorthstyleDate::instance($value);
	}
